...
public class LemonadeStand {
    ...
    public boolean sellLemonade(int discountPercent) {
        ...
        if (tryMakingLemonade(lemonade) != null) {
            double normalPrice = calculateSale(lemonade);
            double discount = normalPrice * (discountPercent / 100.0);
            money += (normalPrice - discount);
            return true;
        } else {
            return false;
        }
    ...
    private double calculateSale(Lemonade lemonade) {
        return (lemonade.getSugar() * pricePerSugar) +
                (lemonade.getIce() * pricePerIce) +
                (lemonade.getLemons() * pricePerLemon);
    }
}
